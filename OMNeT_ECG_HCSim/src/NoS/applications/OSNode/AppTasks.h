...
class IntrDriven_Task :public sc_core::sc_module,virtual public HCSim::OS_TASK_INIT 
{
 ...
 private:
	...
	//OSmodel->profile->load_profile("./profile/" + std::to_string(cores) + "_core/ecg_" +  std::to_string(cores) + "_core.prof");
	OSmodel->profile->load_profile("./profile/1_core/lwip_1_core.prof");

        if(node_id == (simulation_config.cluster)->gateway_id) ecg_gateway(node_id);
        else{ 
            std::string type = (simulation_config.cluster)->edge_type[node_id];
            ecg_edge(node_id);
        }
